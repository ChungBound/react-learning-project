  ...
  return (
    <Card className="p-4 border-dashed">
      <div className="space-y-4">
        <h3 className="text-lg font-medium">useLocalStorage 示例</h3>
        <p className="text-sm text-muted-foreground">数据将保存在浏览器的 localStorage 中</p>

        <div className="space-y-2">
          <label className="block text-sm font-medium">姓名:</label>
          <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="输入姓名" className="max-w-xs" />
        </div>
...
function WindowSizeExample() {
  ...
  return (
    <Card className="p-4 border-dashed">
      <div className="space-y-4">
        <h3 className="text-lg font-medium">useWindowSize 示例</h3>
        <p className="text-sm text-muted-foreground">调整浏览器窗口大小查看变化</p>

        <div className="p-4 bg-muted rounded-md">
          <p>窗口宽度: {size.width || "未知"} px</p>
          <p>窗口高度: {size.height || "未知"} px</p>
        </div>
      </div>
    </Card>
  )
      ...
      <div className="space-y-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>自定义 Hooks 是一种复用状态逻辑的方式。</CardDescription>
          </CardHeader>
          <CardContent>
            <p className="mb-4">
              自定义 Hooks 是一种在 React 组件之间复用状态逻辑的方式，而不需要更改组件结构。它们是以 "use"
  ...
  return { state, customFunction };
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="mb-4">
              自定义 Hook 可以接受任意参数，并且可以返回任何值。它们遵循与内置 Hooks 相同的规则，如只能在函数组件或其他
              Hooks 中调用。
            </p>
      ...
      <button type="button" onClick={() => {
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>一些常见且有用的自定义 Hooks 示例。</CardDescription>
          </CardHeader>
          <CardContent>
    ...
    <div>
      ...
      <p>电话: {data.phone}</p>
        ...
        <Card>
          ...
          <CardContent>
            <p className="mb-4">
              自定义 Hooks 的一个强大特性是它们可以相互组合，创建更复杂的逻辑。这种组合方式使代码更加模块化和可维护。
            </p>
      ...
      <Dashboard data={data} />
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>在 TypeScript 中使用自定义 Hooks。</CardDescription>
          </CardHeader>
          <CardContent>
            <p className="mb-4">
              TypeScript 可以为自定义 Hooks 提供类型安全，使其更加健壮和易于使用。以下是在 TypeScript 中定义和使用自定义
              Hooks 的示例：
            </p>
            <CodeBlock language="tsx">{`import { useState, useEffect } from 'react';
    ...
    <div>
      ...
      <button onClick={reset}>重置</button>
        ...
        <Card>
          ...
          <CardContent>
            <p className="mb-4">
              测试自定义 Hooks 可以使用 <code>@testing-library/react-hooks</code> 库，它提供了一种简单的方法来测试 Hooks
              的行为，而不需要创建完整的组件。
            </p>
        ...
        <Card>
          ...
          <CardContent>
            <ul className="list-disc list-inside space-y-3">
              <li>
                <strong>命名清晰</strong>：使用描述性的名称，以 "use" 开头，清晰表明 Hook 的用途。
              </li>
              <li>
                <strong>单一职责</strong>：每个 Hook 应该只做一件事，并做好这件事。
              </li>
              <li>
                <strong>组合而非复杂</strong>：创建小型、可组合的 Hooks，而不是大型、复杂的 Hooks。
              </li>
              <li>
                <strong>文档化</strong>：为 Hook 添加注释，说明其用途、参数和返回值。
              </li>
              <li>
                <strong>错误处理</strong>：妥善处理可能的错误，提供有用的错误信息。
              </li>
              <li>
                <strong>性能考虑</strong>：注意 Hook 的性能影响，避免不必要的重新渲染。
              </li>
              <li>
                <strong>测试</strong>：为 Hook 编写单元测试，确保其行为符合预期。
              </li>
              <li>
                <strong>类型安全</strong>：使用 TypeScript 为 Hook 提供类型定义。
              </li>
            </ul>
            <CodeBlock language="tsx" className="mt-4">{`// 一个遵循最佳实践的自定义 Hook 示例
